...
Raw data from DC current ($I_{dc}$), Voltage ($V_{dc}$), Irradiance ($Irr$), and PV Temperature ($pvT$) are processed into various features.
...
* `data_preparation.ipynb`: Cleaning, normalization, and feature engineering.
...
3.  **Execution Order:**
    - Run `data_preparation.ipynb` to process raw `.mat` files.
