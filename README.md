...
## **5. How to Run**
1.  **Dependencies:** Install via `pip install pandas scikit-learn matplotlib seaborn`.
2.  **Data:** Ensure `pca_8_components.csv` and `labels.csv` are in the root directory.
3.  **Execution:** Run the Jupyter notebooks in sequence: `Preparation` $\rightarrow$ `PCA` $\rightarrow$ `Clustering`.
